...
- `BackgroundGradient` (`src/components/ui/BackgroundGradient.jsx`) is a fixed, `-z-10`, very low-opacity radial glow using `color-mix(in srgb, var(--accent) X%, transparent)` — it's what keeps the flat background from looking like one boring solid color. Rendered once at the top of `App.jsx`. Keep it subtle if you touch it; it should read as atmosphere, not a visible shape.
