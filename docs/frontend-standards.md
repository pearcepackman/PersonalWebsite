...
`BackgroundGradient` was originally `position: fixed` so its glow orbs stayed anchored to the viewport while scrolling. On iOS Safari, `position: fixed` combined with `mix-blend-mode` children corrupts into a flat solid-color block on scroll-direction changes (e.g. the rubber-band bounce at the top) — a real, reproducible bug, not a one-off. A JS-driven `transform: translate3d(...)` synced to `scrollY` (to fake "fixed" without the native fixed-position compositing path) was tried next, but a rAF-throttled JS listener can't keep up with iOS's native momentum/fling scrolling and visibly lags, exposing a gap of plain `--bg` at the edge. The fix that actually held up: make the background span the **full page height** as a normal `position: absolute` element inside a `position: relative` wrapper around all page content, so it scrolls natively with the page — no JS synchronization needed, so neither failure mode is possible. Don't reintroduce `position: fixed` (or a scroll-synced transform standing in for it) for any full-bleed decorative layer without testing on real iOS Safari first, not just desktop/Android.

Sizing that background also needs care: percentage-based gradient sizing (`ellipse 60% 50%`) is relative to the element's *own* box, so once that box is full-page-tall instead of viewport-tall, the ellipses stretch into huge distorted ovals. Size decorative layers like this in `vh`/`vw` (viewport units) instead, anchored near specific page landmarks, not as a percentage of a box that might be much taller than the screen.

Mobile gets no gradient at all (`hidden md:block` on both orbs) rather than a scaled-down version — mobile browser chrome (address bar / bottom toolbar) resizes the visual viewport as it shows/hides, which `vh`-based sizing can't track, so a gap of plain `--bg` reappears every time the chrome toggles. Chasing that further wasn't worth it; flat background + grain texture reads fine on mobile.
